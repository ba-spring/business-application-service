...
public class DefaultWebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.inMemoryAuthentication().withUser("controllerUser").password(encoder.encode("controllerUser1234;")).roles("kie-server","rest-all");
        auth.inMemoryAuthentication().withUser("wbadmin").password(encoder.encode("wbadmin")).roles("admin");
        auth.inMemoryAuthentication().withUser("donato").password(encoder.encode("donato")).roles("kie-server","rest-all","Administrators");
    }
